...
describe('greaterEqual', function () {
    ...
    beforeEach(inject(function (greaterEqualFilter) {
        filter = greaterEqualFilter;
    }));
    ...
    it("should compare to a given field", function () {
        ...
        expect(filter([item], 'sockCount', 3).length).toBe(1);
        expect(filter([item], 'sockCount', 4).length).toBe(1);
        expect(filter([item], 'sockCount', 5).length).toBe(0);
    });
});
